...
let sectionName = function(section, course) {
	return (section.sectionTitle || course.label || course.name)[0];
};

let sectionCode = function(section, course) {
	return (section.sectionNumber || [''])[0]
};
...
let addSectionsToDb = function(course, sections) {
	...
	for(let i = 0; i < sections.length; i++) {
		...
		section_obj['description'] = (section.sectionText || course.description || [''])[0];
...
/*
			...
			let execCourse = function(course) {
				Class.create(course_obj).then(function(res) {
					console.log('Added class ' + res.name);
